...
function login(email, password, rememberMe) {
    ...
    if (user && user.password === password) {
        ...
        // Redirect to projects page instead of index.html
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // If user is on login or register page but already logged in, redirect to projects page
    if ((currentPage === 'login.html' || currentPage === 'register.html') && isLoggedIn()) {
        window.location.href = 'projects.html';
        return;
    }
    
    // If user is on index or projects page but not logged in, redirect to login
    if ((currentPage === 'index.html' || currentPage === 'projects.html') && !isLoggedIn()) {
        window.location.href = 'login.html';
        return;
    }
});

// Define checkAuth if it's not already defined in the file
function checkAuth() {
    const session = JSON.parse(localStorage.getItem('orangeAcademySession'));
    return !!(session && session.loggedIn);
}
